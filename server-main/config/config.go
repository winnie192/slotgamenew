...
var (
	...
	//    go build -ldflags="-X 'github.com/winnie192/slotgame/server/config.BuildVers=%buildvers%'"
	BuildVers string

	// compiled binary build date, sets by compiler with command
	//    go build -ldflags="-X 'github.com/winnie192/slotgame/server/config.BuildTime=%buildtime%'"
	BuildTime string
)
...
var Cfg = &Config{
	...
	CfgSendCode: CfgSendCode{
		...
		SenderName:         "slotgame server",
		SenderEmail:        "slotgame.dev@gmail.com",
		ReplytoEmail:       "noreply@gmail.com",
		EmailSubject:       "slotgame verification code",
		EmailHtmlContent:   "<html><head></head><body><p>Your slotgame verification code is: <b>%06d</b></p></body></html>",
